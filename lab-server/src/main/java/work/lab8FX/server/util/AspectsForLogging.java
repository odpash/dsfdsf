...
public class AspectsForLogging {
    ...
    @After("execution(* work.lab7.server.util.CommandManager.executeServerCommand(..))")
    public void executeServerCommandAdvise(JoinPoint joinPoint) {
        LOGGER.info("Executing server command: " + joinPoint.getArgs()[0]);
    }

    @After("execution(* work.lab7.server.util.CommandManager.executeClientCommand(..))")
    ...
    @After("execution(* work.lab7.server.db.DBManager.*(..))")
